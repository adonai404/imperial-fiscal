...
import { useCompanyFiscalEvolutionData } from '@/hooks/useFiscalData';
...
interface CompanyFiscalEvolutionChartProps {
  companyId: string;
  companyName: string;
  className?: string;
}

export const CompanyFiscalEvolutionChart = ({ 
  companyId, 
  companyName, 
  className 
}: CompanyFiscalEvolutionChartProps) => {
  const { data: evolutionData, isLoading, error } = useCompanyFiscalEvolutionData(companyId);
  ...
  const totals = useMemo(() => {
    if (!chartData.length) return { entrada: 0, saida: 0, imposto: 0 };
